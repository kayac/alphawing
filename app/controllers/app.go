...
func (c AppControllerWithValidation) GetApp(appId int) revel.Result {
	...
	apkBundles, err := app.BundlesByPlatformType(Dbm, models.BundlePlatformTypeAndroid, 100)
	if err != nil {
		panic(err)
	}

	ipaBundles, err := app.BundlesByPlatformType(Dbm, models.BundlePlatformTypeIOS, 100)
	if err != nil {
		panic(err)
	}
...
func (c *AppControllerWithValidation) CheckForbidden() revel.Result {
	...
	if _, err := s.GetBucket(app.FileId); err != nil {
		if err := models.TryDeleteUserApp(Dbm, c.LoginUserId, app.Id); err != nil {
			panic(err)
		}
		return c.Forbidden("Can't access the app.")
	}

	if _, err := models.TryCreateUserApp(Dbm, c.LoginUserId, app.Id); err != nil {
		panic(err)
	}
